...
class ClientsController extends Controller
{
    ...
    public function post(User $user, Request $request){
        ...
        if( isset($input['save']) ||  isset($input['save_and_exit']) ){
			...
			if ($validator->fails()) {
				return redirect()->route('admin.addClient')->withErrors($validator)->withInput();
			}
        ...
        if( isset($input['update']) || isset($input['update_and_exit']) ){
            ...
            if ($validator->fails()) {
				return redirect()->route('admin.viewClient', ['id' => $input['id'] ] )->withErrors($validator)->withInput();
			}
			...
			if( $user->update() ){
                if( isset($input['update_and_exit']) ){
				    return redirect()->route('admin.clients')->with('status','Пользователь обновлен');
                }else{
                    return redirect()->route('admin.viewClient',['id' => $input['id'] ])->with('status','Пользователь обновлен');
                }
			}
        }
        //-----------------------------------------------------------------
        ...
        if( isset($input['dell']) ){
            ...
            $tmp->delete();
            return redirect()->route('admin.clients')->with('status','Пользователь удалён');
        }
        //-----------------------------------------------------------------
        ...
        if( isset($input['search']) && $input['search']!=null ){
            if(view()->exists('admin.clients.list')){
				$search = $input['search'];
				$paginate = 25;
    ...
    public function view($id){
		if(view()->exists('admin.clients.edit')){
    ...
    public function list(Request $request){
		if(view()->exists('admin.clients.list')){
            ...
            $items = User::where('role', '=', '0')->paginate($paginate);
